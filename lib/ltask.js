...
function LTask(task_fn) {
  ...
  this._task_started_children = false
...
LTask.prototype._start_children = function () {
  // Note that we started calling our children.
  this._task_started_children = true
...
//
// Desc:
//  This is essentially a reverse of `this.req`. The difference
//  being that this function sets the completion of *this* task
//  as a requirement for the given task. In the end, all it does
//  is call `task.req(this)`. It's just a shorthand function,
//  designed for readability of code flow.
LTask.prototype.is_req = function (task) {
  if (typeof(task) == 'function')
    task = new LTask(task)
  task.req(this)
  return this
...
// (start_pars) -> undefined
//
// Params:
//  start_pars: If true, start the par'd tasks. Defaults to true.
//
// Desc:
//  Start this task chain. Note that start will not execute if there
//  are any reqs that have not passed
LTask.prototype.start = function (start_pars) {
  if (this._task_started)
    return undefined
  if (start_pars === undefined || start_pars === null)
    start_pars = true
  ...
  if (start_pars) {
    for (var i = 0; i < this._pars.length; i++) {
      // We are calling start with false because all par'd tasks are
      // started together. Since we are starting them, we don't want
      // them also trying to start, and everyone else, and etc.
      this._pars[i].start(false)
    }
  }
  return undefined
}
...
//
// Desc:
//  Check whether or not this ltask has started it's children or not.
//  This is the last stage, after `this.completed()`
LTask.prototype.started_children = function () {
  return this._task_started_children
}
